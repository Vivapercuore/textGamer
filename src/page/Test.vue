...
const wuxingjineng = `沉默誓言 孤注一掷 狂怒血脉 胜利军规 敏锐洞察 胜利的代价 侵蚀之焰 波澜攻势 怒火猛攻 风卷残云 毁灭的预示 三重攻势 军神赠礼 破敌之眼 反戈一击 灵光乍现 避芒击惰 王者之剑 弱点进攻 英勇时刻 决意强袭 压迫之袭 天地无双斩 正义裁决 恩泽庇护 光明护佑 战术冲锋 冲锋的极意 致命一击 慈悲终结 饥渴之刃 百战之体以战养战 燎天之火 泰山压顶 胜利怒吼 蛮勇侵袭`.split(' ')
...
<template>
  ...
  <el-alert
    title="只是随手做的,根据官方公示的概率和大致的猜测,仅供娱乐,和实际并不一致,只做了一个卡池,传奇现世"
    type="warning">
  </el-alert>
  <el-alert
    title="按照官方公布的概率,并假设所有同星级出货概率相同."
    type="warning">
  </el-alert>
  <el-alert
    title="因为保底策略让你抽中的,会标识(保底)."
    type="warning">
  </el-alert>
  <a href="https://dg.qq.com/webplat/info/news_version3/58876/58877/m22333/202109/900062.shtml">官方标的概率</a>
    <el-main>
      <el-row :gutter="20" >
        ...
        <!-- <el-button>抽到 xx 为止</el-button> -->
        <el-button @click="reset">清空重来</el-button>
      </el-row>
      <el-row :gutter="20"  v-show="resList.length>0">
        <el-card>
          你已经抽了 {{resList.length}} 次了, 花了{{resList.length * 198}}银币 ,{{( resList.length * 19.8).toFixed(2)}}块钱打了水漂
        </el-card>
      </el-row>
      <h1>抽到的卡:</h1>
      <el-row :gutter="20">
        <el-col
          :span="24"
        >
          <el-card class="scenarioItem"  @click="chooseScenario(scenarioItem)" v-for="(res,index) in showList" :key="index">
      ...
      <el-row :gutter="5">
        <el-col
          :span="24"
        >
          <el-card class="scenarioItem" @click="chooseScenario(scenarioItem)" v-for="(res,index) in resList" :key="index">
